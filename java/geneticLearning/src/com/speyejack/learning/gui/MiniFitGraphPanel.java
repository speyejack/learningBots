...
public class MiniFitGraphPanel extends JPanel {
	...
	public MiniFitGraphPanel(Dimension dim, Pool pool) {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 1; i < dataCache.size(); i++) {
			...
			g.drawLine(adjustWidth(dataCache.get(i)[0]), adjustHeight(dataCache.get(i)[1]), adjustWidth(dataCache.get(i)[0]), adjustHeight(dataCache.get(i)[3]));
		...
		for (int i = 1; i < Y_SCALAR; i++){
			...
			g.drawString(Integer.toString((int) (maxFit * 1.1 - (i * maxFit * 1.1 / Y_SCALAR))), (int) (0.011 * dim.getWidth()), height + 5);
		}
